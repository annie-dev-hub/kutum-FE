...
import { Plus, Edit, X, Trash2, MessageCircle, Send, Minimize2 } from 'lucide-react'
...
export default function PeoplePage() {
  ...
  const [chatMessages, setChatMessages] = useState<ChatMessage[]>([
    {
      id: '1',
      role: 'assistant',
      content: 'Hello! I can help you with information about your family members. Try asking me things like:\n• "How many children do I have?"\n• "What is Rajesh\'s blood group?"\n• "Tell me about my spouse"',
  ...
  const dispatchActivity = (action: string, description: string) => {
    window.dispatchEvent(new CustomEvent('activityAdded', {
      detail: { type: 'family', action, description }
    }))
  }
  ...
  const handleAddMember = () => {
    ...
    // Dispatch activity
    dispatchActivity('added', `Added new family member: ${memberToAdd.name}`)
    setNewMember({
      name: '',
      relation: 'Son',
      age: '',
  ...
  const handleUpdateMember = () => {
    ...
    // Dispatch activity
    dispatchActivity('updated', `Updated family member: ${editMember.name}`)
  }
  ...
  const handleDeleteMember = (memberId: string) => {
    ...
    if (confirm(`Are you sure you want to delete ${memberToDelete?.name}?`)) {
      ...
      // Dispatch activity
      if (memberToDelete) {
        dispatchActivity('deleted', `Deleted family member: ${memberToDelete.name}`)
      }
      
      // Update selected member if deleted member was selected
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button 
                      onClick={() => handleDeleteMember(selected.id)}
                      className="flex items-center gap-2 px-3 py-2 rounded-lg bg-red-100 hover:bg-red-200 text-red-700"
                    >
      ...
      {!showChat && (
        <div className="fixed bottom-24 right-6">
          <button 
            ...
            title="Chat with Family Assistant"
          >
