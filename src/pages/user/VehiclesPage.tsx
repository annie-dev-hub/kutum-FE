...
export default function VehiclesPage() {
  ...
  const dispatchActivity = (action: string, description: string) => {
    window.dispatchEvent(new CustomEvent('activityAdded', {
      detail: { type: 'vehicle', action, description }
    }));
  };
  ...
  const handleSaveVehicle = (vehicleData: Vehicle) => {
    const isEdit = rawVehicles.find(v => v.id === vehicleData.id);
    ...
    const description = isEdit ? `Updated vehicle: ${vehicleData.name}` : `Added vehicle: ${vehicleData.name}`;
    dispatchActivity(action, description);
  };
  ...
  const confirmDelete = () => {
    if (vehicleToDelete) {
      const vehicleToRemove = rawVehicles.find(v => v.id === vehicleToDelete);
      setRawVehicles(prev => prev.filter(v => v.id !== vehicleToDelete));
      
      // Dispatch activity
      if (vehicleToRemove) {
        dispatchActivity('deleted', `Deleted vehicle: ${vehicleToRemove.name}`);
      }
    }
