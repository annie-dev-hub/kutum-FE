...
export const CLOTHING_SIZES_STORAGE_KEY = 'kutum_clothing_sizes'
...
const PREDEFINED: ClothingSize[] = [
  { id: 1, name: 'XS', description: 'Default', status: 'Active', isPredefined: true },
  { id: 2, name: 'S', description: 'Default', status: 'Active', isPredefined: true },
  { id: 3, name: 'M', description: 'Default', status: 'Active', isPredefined: true },
  { id: 4, name: 'L', description: 'Default', status: 'Active', isPredefined: true },
  { id: 5, name: 'XL', description: 'Default', status: 'Active', isPredefined: true },
  { id: 6, name: 'XXL', description: 'Default', status: 'Active', isPredefined: true },
]
...
export default function ClothingSizesPage() {
  ...
  const [rows, setRows] = useState<ClothingSize[]>(() => {
    try {
      const stored = localStorage.getItem(CLOTHING_SIZES_STORAGE_KEY)
  ...
  const addNew = () => {
    ...
    if (!name) return
    const next: ClothingSize = {
      id: rows.length ? Math.max(...rows.map((r) => r.id)) + 1 : 1,
      name,
      description: form.description.trim() || `${name} size`,
      status: 'Active',
      isPredefined: false,
    }
    setRows((r) => [...r, next])
  ...
  const startEdit = (row: ClothingSize) => {
  ...
  useEffect(() => {
    localStorage.setItem(CLOTHING_SIZES_STORAGE_KEY, JSON.stringify(rows))
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/30 grid place-items-center p-4">
          <div className="w-full max-w-lg card p-6 relative">
            ...
            <h3 className="text-xl font-semibold mb-4">{editingId ? 'Edit Clothing Size' : 'Add Clothing Size'}</h3>
            <div className="space-y-4">
              <div>
                <label className="text-sm font-medium text-slate-700">Name</label>
                <input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="mt-1 w-full rounded-xl bg-slate-100/80 px-4 py-3 outline-none focus:ring-2 ring-primary" placeholder="e.g. XXL" />
              </div>
              <div>
