...
export const BLOOD_GROUPS_STORAGE_KEY = 'kutum_blood_groups'
...
const PREDEFINED: BloodGroup[] = [
  { id: 1, name: 'A+', description: 'Default', status: 'Active', isPredefined: true },
  { id: 2, name: 'A−', description: 'Default', status: 'Active', isPredefined: true },
  { id: 3, name: 'B+', description: 'Default', status: 'Active', isPredefined: true },
  { id: 4, name: 'O+', description: 'Default', status: 'Active', isPredefined: true },
  { id: 5, name: 'AB+', description: 'Default', status: 'Active', isPredefined: true },
  { id: 6, name: 'O−', description: 'Default', status: 'Active', isPredefined: true },
]
...
export default function BloodGroupsPage() {
  ...
  const [rows, setRows] = useState<BloodGroup[]>(() => {
    try {
      const stored = localStorage.getItem(BLOOD_GROUPS_STORAGE_KEY)
  ...
  const addNew = () => {
    ...
    if (!name) return
    const next: BloodGroup = {
      id: rows.length ? Math.max(...rows.map((r) => r.id)) + 1 : 1,
      name,
      description: form.description.trim() || `${name} blood type`,
      status: 'Active',
      isPredefined: false,
    }
    setRows((r) => [...r, next])
  ...
  const startEdit = (row: BloodGroup) => {
    setEditingId(row.id)
  ...
  useEffect(() => {
    localStorage.setItem(BLOOD_GROUPS_STORAGE_KEY, JSON.stringify(rows))
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <button onClick={() => setShowForm(true)} className="gradient-btn px-3 sm:px-4 py-2 flex items-center gap-2">
            <Plus />
            <span>Add New</span>
          </button>
        </div>
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/30 grid place-items-center p-4">
          <div className="w-full max-w-lg card p-6 relative">
            ...
            <h3 className="text-xl font-semibold mb-4">{editingId ? 'Edit Blood Group' : 'Add Blood Group'}</h3>
            <div className="space-y-4">
              <div>
                <label className="text-sm font-medium text-slate-700">Name</label>
                <input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="mt-1 w-full rounded-xl bg-slate-100/80 px-4 py-3 outline-none focus:ring-2 ring-primary" placeholder="e.g. A+" />
              </div>
              <div>
